...
export default function TIABarDashboard() {
  // Index both years by ID ("CAMPUS_ID")
  const map23 = useMemo(() => new Map(data23.map((r) => [r.CAMPUS_ID, r])), []);
  const map24 = useMemo(() => new Map(data24.map((r) => [r.CAMPUS_ID, r])), []);
  ...
  const DEFAULT_CAMPUS_ID = "'015912001"; // e.g., SOUTHWEST H S
  const [selectedId, setSelectedId] = useState(
    data23.find((r) => r.CAMPUS_ID === DEFAULT_CAMPUS_ID)?.CAMPUS_ID ?? data23[0]?.CAMPUS_ID ?? null
  );
    ...
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        ...
        <FormControl size="small" sx={{ minWidth: 280 }}>
          ...
          >
            {campuses.map((c) => (
              <MenuItem key={c.CAMPUS_ID} value={c.CAMPUS_ID}>
                {c.label} ({c.CAMPUS_ID})
              </MenuItem>
            ))}
          </Select>
